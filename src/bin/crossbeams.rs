...
impl MutMgr {
    fn run(sender: mpsc::Sender<i32>, receiver: mpsc::Receiver<i32>) {
        let mut count = 0;
        let mut_pool = ThreadPool::new(1);
        let send_pool = ThreadPool::new(1);
        ...
        thread::spawn(move ||
            loop {
                ...
                if let Ok(item) = receiver.try_recv() {
                    let tx = tx.clone();
                    mut_pool.execute(move || {
                        println!("MutMgr received: {}, {}/{}", item, mut_pool.active_count(), mut_pool.max_count());
        ...
        loop {
            if let Ok(item) = rx.try_recv() {
                ...
                send_pool.execute(move || {
                    println!("MutMgr sending: {} {}/{}", item, mut_pool.active_count(), mut_pool.max_count());
...
impl RunMgr {
    fn run(sender: mpsc::Sender<i32>, receiver: mpsc::Receiver<i32>) {
        let mut_pool = ThreadPool::new(3);
        let send_pool = ThreadPool::new(3);
        let (tx, rx): (mpsc::Sender<i32>, mpsc::Receiver<i32>) = channel();
        let mut count = 0;
        // receive
        thread::spawn(move ||
                ...
                match receiver.try_recv() {
                    Ok(item) => {
                        ...
                        mut_pool.execute(move || {
                            ...
                            println!("RunMgr received: {} {}/{}", item, mut_pool.active_count(), mut_pool.max_count());
