...
named!(
    key<(String, Option<u8>)>,
    map!(
        re_bytes_capture!(r"([[:alpha:]][[:word:]]*)(?:(?:@)(\d+))?"),
        |v| {
...
named!(
    hex_part<Vec<u8>>,
    many1!(map_res!(
        map_res!(preceded!(tag!("\\x"), take!(2)), str::from_utf8),
        |src| u8::from_str_radix(src, 16)
    ))
);
...
named!(
    ...
    delimited!(
        tag!("\""),
        fold_many1!(
            alt!(hex_part | char_part),
            Vec::new(),
            |mut acc: Vec<_>, ref item: Vec<_>| {
...
named!(
    comment<&str>,
    map_res!(
        preceded!(tag!("#"), ws!(call!(not_line_ending))),
        str::from_utf8
    )
);
...
fn test_hex_str() {
    ...
    let kv = kv(&r#"header_png@2 = "\x89PNG\x0d\x0a\x1a\x0a""#.as_bytes()[..]);
    println!("kv={:?}", kv);
}
